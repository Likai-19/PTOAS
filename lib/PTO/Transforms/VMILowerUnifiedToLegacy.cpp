...
//   vload  → dispatch by dist_mode to load/deinterleave_load
//   vstore → dispatch by dist_mode to store/interleave_store
//   Skipped: dist_mode "unpack", "brc" (no simple legacy equivalent).
//
// Category C4 — static mask creation (2 ops):
...
//            nor reduce_mini)
//
// Category D — no legacy equivalent (explicitly skipped, ~14 ops):
//   plt vintlv vdintlv vselr vgather vgatherb
//   vscatter vexpdif vaxpy vlrelu vprelu vmull vmula vhist
//
//===----------------------------------------------------------------------===//
...
static LogicalResult lowerVLoad(VMIvLoadOp op, OpBuilder &builder) {
  ...
  if (op.getBlockStrideAttr()) {
    ...
    Value bs = builder.create<arith::ConstantOp>(
        op->getLoc(), i16Type,
        builder.getIntegerAttr(i16Type, op.getBlockStrideAttr().getInt()));
    Value rs = builder.create<arith::ConstantOp>(
        op->getLoc(), i16Type,
        builder.getIntegerAttr(i16Type, op.getRepeatStrideAttr().getInt()));
  ...
  if (distMode == "continuous") {
    ...
    // "unpack" and "brc" have no simple legacy equivalent; skip.
    return failure();
  }
...
static LogicalResult lowerVStore(VMIvStoreOp op, OpBuilder &builder) {
  ...
  // pmode="merge" cannot be expressed by legacy store — skip.
  if (hasMergePmode(op))
    return failure();
