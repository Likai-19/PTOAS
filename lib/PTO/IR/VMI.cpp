...
ParseResult VMIvStoreOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  OpAsmParser::UnresolvedOperand postBracketOp1;
  OpAsmParser::UnresolvedOperand postBracketOp2;
  bool hasPost1 = false;
  bool hasPost2 = false;
  ...
  // Optional post-bracket operands: stride and/or mask.
  // Disambiguated after parsing attrs (group present → stride, else mask).
  if (succeeded(parser.parseOptionalComma())) {
    if (parser.parseOperand(postBracketOp1))
      return failure();
    hasPost1 = true;
    if (succeeded(parser.parseOptionalComma())) {
      if (parser.parseOperand(postBracketOp2))
        return failure();
      hasPost2 = true;
    }
  }
  ...
  bool hasStride = hasPost1 && hasGroup;
  bool hasMask = hasGroup ? hasPost2 : hasPost1;
  ...
  if (hasStride &&
      parser.resolveOperand(postBracketOp1, parser.getBuilder().getIndexType(),
                            result.operands))
    return failure();

  if (hasMask) {
    Type maskType = types.back();
    auto &maskOperand = hasGroup ? postBracketOp2 : postBracketOp1;
    if (parser.resolveOperand(maskOperand, maskType, result.operands))
      return failure();
  }
...
void VMIvStoreOp::print(OpAsmPrinter &p) {
  ...
  p.printOptionalAttrDict((*this)->getAttrs(), {"dist_mode", "group", "pmode"});
  p << " : ";
  for (auto val : getValues())
    p << val.getType() << ", ";
...
LogicalResult VMIvStoreOp::verify() {
  ...
  // dist_mode and group
  bool hasBlock = getBlockStride().has_value();
  bool hasRepeat = getRepeatStride().has_value();
...
ParseResult VMIvLoadOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  // Optional stride operand: vload %src[%off], %stride {group = C}
  bool hasStride = succeeded(parser.parseOptionalComma());
  if (hasStride && parser.parseOperand(strideOperand))
    return failure();
