...
int main() {
  ...
  size_t srcBytes = kElems * sizeof(uint32_t);
  size_t dstBytes = kElems * sizeof(uint16_t);
  uint32_t *srcHost = nullptr;
  uint16_t *dstHost = nullptr;
  uint32_t *srcDevice = nullptr;
  ...
  LaunchVstore_pack_ui32_to_ui16_kernel(srcDevice, dstDevice, stream);
