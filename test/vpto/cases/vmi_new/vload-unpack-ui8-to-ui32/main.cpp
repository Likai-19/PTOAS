...
int main() {
  ...
  size_t dstBytes = kElems * sizeof(uint32_t);
  uint8_t *srcHost = nullptr;
  uint32_t *dstHost = nullptr;
  uint8_t *srcDevice = nullptr;
  uint32_t *dstDevice = nullptr;
  int rc = 0; bool aclInited = false; bool deviceSet = false;
  ...
  LaunchVload_unpack_ui8_to_ui32_kernel(srcDevice, dstDevice, stream);
